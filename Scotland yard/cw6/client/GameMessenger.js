...
GameMessenger.prototype.interpretReady = function (messageReady) {
  ...
  if(AIPlayers.length > 0 && aiMessenger.isConnected()) aiMessenger.sendMessage(messageReady);
};
...
GameMessenger.prototype.interpretNotify = function (messageNotify) {
  //TODO:
  if(AIPlayers.length > 0 && aiMessenger.isConnected()) aiMessenger.sendMessage(messageNotify);
  if(messageNotify['move_type'] == 'MoveTicket') {
    guiConnector.animatePlayer(messageNotify['move']['colour'], messageNotify['move']['target']);
    guiConnector.removeTicket(messageNotify['move']['colour'], messageNotify['move']['ticket']);
    if(messageNotify['move']['colour'] == 'Black') guiConnector.updateTicketView(messageNotify['move']['ticket'],messageNotify['move']['target']);
  }
  else if(messageNotify['move_type'] == 'MoveDouble') guiConnector.removeTicket(messageNotify['move']['colour'],"Double");
};
...
GameMessenger.prototype.interpretGameOver = function (messageGameOver) {
  ...
  if(AIPlayers.length > 0 && aiMessenger.isConnected()) aiMessenger.sendMessage(messageGameOver);
};
...
GameMessenger.prototype.interpretConnection = function (messageConnection) {
  ...
  var message = {
    "game_id": guiConnector.getSelectedGame(),
    "type": "SPECTATE"
  };
...
GameMessenger.prototype.interpretNotifyTurn = function (messageNotifyTurn) {
  //TODO:
  if(AIPlayers.length > 0 && aiMessenger.isConnected() && AIPlayers.indexOf(messageNotifyTurn['valid_moves'][0]['move']['colour']) != -1){
    aiMessenger.sendMessage(messageNotifyTurn)
    guiConnector.startTurn(messageNotifyTurn, true);
  }
  else guiConnector.startTurn(messageNotifyTurn, false);
};
